...
def extraction(file_path, indicator):
    """
    extracts the named entities or sentiments depending on the indicator (if indicator NER then it will extract NER and vice versa)
    """
...
def main():
    ...
    by using command line input each book gets processed one by one
    """
    ...
    for i in range(0, amount):
        folder_path = input("Enter the path of the folder of the book containing the files to be processed: ")
        try: 
            for f in os.listdir(folder_path):
                f_path = os.path.join(folder_path, f)
                NER = extraction(f_path, "NER")
                SENT = extraction(f_path, "SENT")
            output_path = input("Please enter the path where the json files should be stores at: ")
            json_maker(NER, output_path, "NER")
            json_maker(SENT, output_path, "SENT")
            print("all done!")
        except:
